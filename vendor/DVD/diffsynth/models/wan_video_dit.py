...
def precompute_freqs_cis(dim: int, end: int = 1024, theta: float = 10000.0):
    # 1d rope precompute (float32: MPS has no float64 / complex128)
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)
                   [: (dim // 2)].float() / dim))
    freqs = torch.outer(torch.arange(end, device=freqs.device).float(), freqs)
...
def rope_apply(x, freqs, num_heads):
    x = rearrange(x, "b s (n d) -> b s n d", n=num_heads)
    # float64 is unsupported on MPS; float32 RoPE is fine for inference
    x_out = torch.view_as_complex(
        x.to(torch.float32).reshape(x.shape[0], x.shape[1], x.shape[2], -1, 2)
    )
    if freqs.dtype != x_out.dtype:
        freqs = freqs.to(dtype=x_out.dtype)
