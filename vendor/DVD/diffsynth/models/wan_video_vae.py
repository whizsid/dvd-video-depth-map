...
class VideoVAE_(nn.Module):
    ...
    def decode(self, z, scale):
        ...
        iter_ = z.shape[2]
        print(
            f"      VAE decode conv2 on {z.device.type} "
            f"(latent T={iter_}, shape={tuple(z.shape)})...",
            flush=True,
        )
        t_conv = time.perf_counter()
        x = self.conv2(z)
        is_mps = getattr(z.device, "type", "") == "mps"
        sync_every = 4
        if is_mps:
            try:
                sync_every = max(1, int(os.environ.get("DVD_VAE_MPS_SYNC_EVERY", "4")))
            except ValueError:
                sync_every = 4
            try:
                ...
                pass
        print(
            f"      VAE decode conv2 done ({time.perf_counter() - t_conv:.1f}s)",
            flush=True,
        )
        for i in range(iter_):
            self._conv_idx = [0]
            # Progress so long decodes do not look hung (8GB Mac: many minutes/slice on CPU).
            print(
                f"      VAE decode temporal {i + 1}/{iter_} ({z.device.type})...",
                flush=True,
            )
            ...
            # Thin MPS sync: every N slices + always the last (numerics unchanged).
            if is_mps and ((i + 1) % sync_every == 0 or i + 1 == iter_):
                try:
                    torch.mps.synchronize()
                except Exception:
                    pass
            print(
                f"      VAE decode temporal {i + 1}/{iter_} done "
                f"({time.perf_counter() - t_slice:.1f}s)",
                flush=True,
            )
        return out
