...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        ...
        self._zero_context_cache = {}

    def process(self, pipe: WanVideoPipeline, prompt, positive, prompt_num) -> dict:
        key = (int(prompt_num), str(pipe.torch_dtype), str(pipe.device))
        cached = self._zero_context_cache.get(key)
        if (
            cached is None
            or cached.device != pipe.device
            or cached.dtype != pipe.torch_dtype
            or cached.shape[0] != prompt_num
        ):
            cached = torch.zeros(
                [prompt_num, 512, 4096],
            ...
            self._zero_context_cache[key] = cached
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(
        ...
        # Quality-safe reuse: full-frame extra_images with all-ones index means
        # the RGB encoded into latents matches what we would encode for y.
        latent_len = (num_frames - 1) // 4 + 1
        can_reuse_latents = (
            torch.is_tensor(latents)
            and extra_images is not None
            and extra_image_frame_index is not None
            and bool(torch.all(extra_image_frame_index == 1))
            and latents.ndim == 5
            and latents.shape[0] == batch_size
            and latents.shape[2] == latent_len
            and latents.shape[3] == height // 8
            and latents.shape[4] == width // 8
        )
        if can_reuse_latents:
            y = latents.to(dtype=pipe.torch_dtype, device=pipe.device)
        else:
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    _shortcut = latents
    # Reuse RoPE freqs across windows with the same latent grid.
    _freqs_cache = getattr(dit, "_dvd_rope_freqs_cache", None)
    _freqs_key = (int(f), int(h), int(w), str(latents.device), str(latents.dtype))
    if (
        isinstance(_freqs_cache, tuple)
        and len(_freqs_cache) == 2
        and _freqs_cache[0] == _freqs_key
        and torch.is_tensor(_freqs_cache[1])
        and _freqs_cache[1].device == latents.device
    ):
        freqs = _freqs_cache[1]
    else:
        freqs = (
            torch.cat(
                [
        ...
        dit._dvd_rope_freqs_cache = (_freqs_key, freqs)
